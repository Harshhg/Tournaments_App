...
function Matches(key) {
    ...
    function handleMatches(match_id,player1,player2,player1name,player2name,iscomplete,image1,image2){
        if(player2name==="null" || iscomplete===true){
            ...
            <div  id={item.round_no} key={item.round_no}>
                {
                    item.match_data.map(matchitem=>
                        <div className="roundtab" key={matchitem.match_id} onClick={()=>handleMatches(matchitem.match_id,matchitem.player1.id,matchitem?.player2?.id,matchitem.player1.name,matchitem?.player2?.name || "null",matchitem.is_completed,matchitem?.player1?.profile_image || blankimage,matchitem?.player2?.profile_image || blankimage)}>
                
                <Row>
                    <Col xs={4} sm={5} md={5} className="text-center">
                        <div>
                    <img src={matchitem?.player1?.profile_image || blankimage} height="80" width="80" alt="Pic" className="tournamentpic" />
                        </div>
                        <div id="firstname" >
                    ...
                    <Col xs={1}  sm={2} md={2} className="text-center p-5">
                        V/S
                    </Col>
                    <Col xs={4} sm={5} md={5} className="text-center">
                        <div>
                    <img src={matchitem?.player2?.profile_image || blankimage} height="80" width="80" alt="Pic" className="tournamentpic" />
                        </div>
                        <div id="secondname" >
